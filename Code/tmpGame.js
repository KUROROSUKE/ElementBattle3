...
async function p2_make() {
    ...
    return new Promise((resolve) => {
        newButton.addEventListener("click", function () {
            ...
            if (p2_make_material) {
                p2_generated_materials.push({
                    name: p2_make_material.name,
                    formula: p2_make_material.formula,
                    point: p2_make_material.point,
                });
            }
...
function downloadRecord() {
    const recordData = {
        dropped_cards_p2: p2_drop,
        generated_materials_p2: p2_generated_materials,
    };
    
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(recordData, null, 2));
    const downloadAnchor = document.createElement("a");
    downloadAnchor.setAttribute("href", dataStr);
    downloadAnchor.setAttribute("download", "p2_game_record.json");
